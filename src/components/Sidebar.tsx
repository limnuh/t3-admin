        ...
        <nav className="mt-5 px-4 py-4 lg:mt-9 lg:px-6">
          ...
          <div>
            ...
            <ul className="mb-6 flex flex-col gap-1.5">
              {/* <!-- Menu Item Settings --> */}
              {menuItems.map(({ path, label, icon, subMenuItems }) =>
                subMenuItems?.length ? (
                  <SidebarLinkGroup activeCondition={pathname.includes(path)} key={path + label}>
                    {(handleClick, open) => {
                      return (
                        <React.Fragment>
                          <Link
                          ...
                          <div className={`translate transform overflow-hidden ${!open ? 'hidden' : ''}`}>
                            <ul className="mb-5.5 mt-4 flex flex-col gap-2.5 pl-6">
                              {subMenuItems.map(({ path: subPath, label: subLabel }) => (
                                <li key={subPath + subLabel}>
                                  <Link
                                    href={subPath}
                                    className={`group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-white ${
                                      pathname.includes(subPath) ? '!text-white' : ''
                                    }`}
                                  >
                                    {subLabel}
                                  </Link>
